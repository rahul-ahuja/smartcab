...
class LearningAgent(Agent):
    ...
    def getQ(self, state, action):
        return self.q.get((state, action), 0.0)  #helper function to get the q-value from the q-table.
	...
	if self.steps_counter == 0:
		...
		oldv = self.q.get((self.state_previous, self.action_previous), None)   #if it's second counter run in the trial then get the q-value from previous state-action  
		try:
			self.q[(self.state,action)] = oldv + self.alpha * (reward - oldv + self.gamma * self.q[(self.state,action)])
		except Exception:
			self.q[(self.state,action)] = reward   #if the state has not been visited before then except statement is triggered otherwise try statement is triggered. 
		#if oldv is None:

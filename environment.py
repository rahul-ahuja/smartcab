...
class Environment(object):
	...
	sucess = self.sucess
        if self.primary_agent is not None:
            ...
            if agent_deadline <= self.hard_time_limit:
                self.done = True
		sucess.append(0)
                print "Environment.step(): Primary agent hit hard time limit ({})! Trial aborted. And the sucess is ({})".format(self.hard_time_limit,sucess)
            elif self.enforce_deadline and agent_deadline <= 0:
                self.done = True
		sucess.append(0)
                print "Environment.step(): Primary agent ran out of time! Trial aborted. And the sucess is ({})".format(sucess)
	...
	self.logfilepath = 'home/rahul/smartcab/results.txt'
	...
	sucess = self.sucess
        ...
        if agent is self.primary_agent:
            if state['location'] == state['destination']:
                ...
                self.done = True
		sucess.append(1)
                print "Environment.act(): Primary agent has reached destination! And the sucess is ({})".format(sucess)  # [debug]
            self.status_text = "state: {}\naction: {}\nreward: {}\nsucess: {}".format(agent.get_state(), action, reward,sucess)
